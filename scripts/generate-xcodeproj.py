...
# Sources that must be in the app target whether or not they exist on disk right now.
#
# BuildSecrets.swift is generated by scripts/generate-secrets.sh and is gitignored
# (US-210). A filesystem walk therefore cannot see it in a fresh checkout, so it would
# never make it into the committed project — and the build would fail at every
# `BuildSecrets.` reference with "cannot find 'BuildSecrets' in scope". Declaring it
# here also makes `--check` deterministic: the result no longer depends on whether a
# developer happens to have generated their secrets yet.
ALWAYS_APP_SOURCES = [
    "Configuration/BuildSecrets.swift",
]
...
def collect() -> dict:
    ...
    for rel in ALWAYS_APP_SOURCES:
        rel_dir = os.path.dirname(rel) or "."
        bucket = app_sources.setdefault(rel_dir, [])
