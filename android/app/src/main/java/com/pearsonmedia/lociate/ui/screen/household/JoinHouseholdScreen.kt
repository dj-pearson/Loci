...
fun JoinHouseholdScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            if (state.currentTier == com.pearsonmedia.loci.domain.model.SubscriptionTier.FREE) {
                Card(
            ...
            if (state.joinSuccess) {
                ...
                OutlinedTextField(
                    ...
                    singleLine = true,
                    modifier = Modifier.fillMaxWidth(),
                    enabled = state.currentTier != com.pearsonmedia.loci.domain.model.SubscriptionTier.FREE
                )

                Button(
                    onClick = { viewModel.joinHousehold(inviteCode) },
                    enabled = inviteCode.isNotBlank() && !state.isLoading &&
                        state.currentTier != com.pearsonmedia.loci.domain.model.SubscriptionTier.FREE,
                    modifier = Modifier.fillMaxWidth()
                ) {
                    if (state.isLoading) {
