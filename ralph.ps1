...
if (-not (Test-Path (Join-Path $ProjectDir ".git"))) {
    ...
    git init
    git add CLAUDE.md prd.json progress.txt ralph-prompt.md ralph.ps1 ralph.sh .gitignore .claude/settings.local.json
    git commit -m "Initial commit: Ralph project scaffolding`n`nCo-Authored-By: Ralph <ralph@snarktank.dev>"
    Pop-Location
    Write-Host ""
}
...
try {
    while ($iteration -lt $MaxIterations) {
        ...
        $claudeArgs = @("-p", $prompt, "--max-turns", $MaxTurns, "--verbose")
