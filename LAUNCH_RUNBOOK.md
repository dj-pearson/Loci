# Lociate — Launch Runbook

End-to-end walkthrough to take Lociate from an empty Apple Developer account + fresh Coolify VPS all the way to "submitted for review" on the App Store. This document is the operational companion to `DEPLOYMENT.md` — where that doc covers the *mechanics* of Fastlane Match, code signing, and the CI workflows, this one covers the *content* (App Store metadata, subscription products, descriptions) and the *deployment targets* (Coolify service env vars).
...
| Domain registrar | `lociate.app` (or your chosen domain) | ~$12/year |

### Domain setup (do this first)

You need at least two subdomains pointing at your Contabo VPS:

- `api.lociate.app` → Supabase Kong gateway
- `functions.lociate.app` → edge functions sidecar
...
| Field | Value |
|---|---|
| Description | Lociate App Group |
| Identifier | `group.app.lociate.ios` |
...
1. Keys → `+` → name `Lociate APNs Key`, enable **Apple Push Notifications service (APNs)**
...
2. Description: `Lociate Web Auth`; Identifier: `app.lociate.ios.signin`
3. Enable **Sign in with Apple**, configure:
   - Primary App ID: `app.lociate.ios`
   - Domain: `api.lociate.app`
   - Return URL: `https://api.lociate.app/auth/v1/callback`

### 2.6 Sign in with Apple — Private Key

1. Keys → `+` → name `Lociate Sign in with Apple`, enable **Sign in with Apple**
2. Configure — primary App ID: `app.lociate.ios`
...
1. Create a **private** GitHub repo `lociate-certificates` (empty).
2. `cd Lociate && fastlane match init` → choose git, paste repo URL.
...
| Privacy Policy URL | `https://lociate.app/privacy` |
| Marketing URL (optional) | `https://lociate.app` |
...
This is the data collection questionnaire. **Answer accurately** — see [Appendix C](#appendix-c--privacy-questionnaire-answers) for exact answers matching Lociate's architecture.
...
- **Support URL**: `https://lociate.app/support`
- **Marketing URL**: `https://lociate.app`
...
The Lociate app has **6 products** across **3 subscription groups**. Create them in order.
...
### 4.2 Create Subscription Group: **Lociate Premium**

Features → Subscriptions → `+` Subscription Group → name `Lociate Premium`.
...
| Field | Value |
|---|---|
| Reference Name | Lociate Premium Monthly |
...
| Localized Display Name | Lociate Premium (Monthly) |
...
| Field | Value |
|---|---|
| Reference Name | Lociate Premium Yearly |
| Product ID | `premium_yearly` |
| Subscription Duration | 1 Year |
| Price | $29.99 USD (≈ 37% savings) |
| Free Trial | 7 days |
| Family Sharing | Off |
| Localized Display Name | Lociate Premium (Yearly) |
| Localized Description | See Appendix B |

### 4.3 Create Subscription Group: **Lociate Family**

Features → Subscriptions → `+` Subscription Group → name `Lociate Family`.

#### Product: `family_monthly`

| Field | Value |
|---|---|
| Reference Name | Lociate Family Monthly |
...
| Field | Value |
|---|---|
| Reference Name | Lociate Family Yearly |
...
Lociate has two lifetime SKUs per `RevenueCatConfiguration.swift`. Configure as **Non-Consumable In-App Purchases**, not subscriptions.
...
| Field | Value |
|---|---|
| Reference Name | Lociate Lifetime Individual |
...
| Field | Value |
|---|---|
| Reference Name | Lociate Lifetime Family |
...
```swift
// Lociate/Services/RevenueCatConfiguration.swift
// Offering IDs:
...
- URL: `https://functions.lociate.app/api/webhook/revenuecat`
...
2. Create Organization → App: `Lociate iOS`
...
- `APPLE_CLIENT_ID` = `app.lociate.ios` (the native app bundle ID — NOT the Services ID)
...
const clientId = 'app.lociate.ios';
...
Lociate's backend is defined in `backend/docker-compose.yml` — Postgres+PostGIS, GoTrue, PostgREST, Realtime, Storage, Kong gateway, plus the edge-functions sidecar.

### 8.1 Coolify Project Setup

1. SSH into Contabo VPS, install Coolify if not done: <https://coolify.io/docs/installation>
2. In Coolify UI → **New Project** → `Lociate Production`
...
2. Source: Public Repository → `https://github.com/pearsonmedia/lociate-ios`
3. Base directory: `/backend`
4. Compose file: `docker-compose.yml`
5. Domains:
   - Map `kong` service port 8000 → `https://api.lociate.app`
   - Map `edge-functions` service port 3000 → `https://functions.lociate.app` *(or deploy edge functions as a separate resource — see §9)*

**Option B — Coolify-native Supabase template:**

Coolify ships a Supabase one-click. If you use it, you'll miss the Lociate-specific edge-functions service and the PostGIS extension. Stick with Option A.
...
| `API_EXTERNAL_URL` | `https://api.lociate.app` | Full public URL including https |
| `SITE_URL` | `https://lociate.app` | Redirect after auth |
| `ADDITIONAL_REDIRECT_URLS` | `lociate://auth-callback` | The native app's URL scheme |
| `APPLE_CLIENT_ID` | `app.lociate.ios` | Bundle ID, not Services ID |
...
```
GET https://api.lociate.app/auth/v1/settings
```
...
| `SUPABASE_URL` | `http://kong:8000` (internal) **or** `https://api.lociate.app` (if on different network) | If bundled: use internal DNS `http://kong:8000` for zero latency |
...
All served under `https://functions.lociate.app`:
...
### Backend
- [ ] `https://api.lociate.app/auth/v1/settings` returns 200 with `apple` provider
- [ ] `https://api.lociate.app/rest/v1/` returns 200 with Swagger JSON
- [ ] `https://functions.lociate.app/api/health` returns 200
...
```
Capture fleeting thoughts at the places that matter. Lociate plays your voice notes back when you return — no folders, no searching, just memory where you left it.
```

### Description (4000 chars max)

```
Lociate is the voice-first memory app for the places in your life.

Some thoughts only make sense where you had them. The hardware store where you noticed the cracked step. The trail where a song finally clicked. The parking spot you'll absolutely forget in four hours. Lociate pins a voice note to that exact spot — and when you come back, it plays itself.
...
• PROXIMITY PLAYBACK — Lociate watches for the pins you've made. Walk within range of one, get a gentle notification — or open the map and see them all.

• PRIVATE BY DEFAULT — No social feed. No public profiles. No "who else was here." Lociate is your memory, for you.
...
Privacy matters. Lociate stores audio encrypted at rest, transcribes on-device by default, and has no ads or trackers — ever.

---

Terms of Use: https://lociate.app/terms
Privacy Policy: https://lociate.app/privacy
```
...
This is the first release. We'd love your feedback — email support@lociate.app.
```
...
|---|---|---|
| `premium_monthly` | Lociate Premium Monthly | Unlimited places, sync, search, widget |
| `premium_yearly` | Lociate Premium Yearly | Unlimited places plus 37% off monthly |
| `family_monthly` | Lociate Family Monthly | Premium for up to 6 family members |
| `family_yearly` | Lociate Family Yearly | Family plan, 30% off monthly price |
| `lifetime_individual` | Lociate Lifetime | Premium forever — no subscription |
| `lifetime_family` | Lociate Family Lifetime | Family plan forever — no subscription |
...
**Does your app track users?** → **No**. Lociate uses no cross-app tracking, no IDFA, no ad networks.
